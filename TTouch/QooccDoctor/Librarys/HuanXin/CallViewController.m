...
- (void)setUserLogo:(NSString *)userLogo{
     [_headerImageView sd_setImageWithURL:[NSURL URLWithString:userLogo] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
}
...
- (void)_setupSubviews
{
    ...
    [_headerImageView sd_setImageWithURL:[NSURL URLWithString:self.userLogo] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
    ...
    _answerButton.layer.cornerRadius = 5;
    ...
    _hangupButton.layer.cornerRadius = 5;
}
...
- (void)_initializeCamera
{
    ...
    _smallView.layer.borderColor = (__bridge CGColorRef)([UIColor whiteColor]);
    _smallView.layer.cornerRadius = 3;
    _smallView.layer.masksToBounds = YES;
    _smallView.layer.borderWidth = 2;
    ...
    if(!error){
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"错误" message:error.localizedFailureReason delegate:nil cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
        [alertView show];
    }
...
//    }
    ...
    if (callSession.status == eCallSessionStatusDisconnected) {
        ...
        if(_timeLength == 0)
        {
            ...
            else if (reason == eCallReason_Null){
            ...
            }else if (reason == eCallReason_Offline) {
                str = @"不在线";
            }
        }
