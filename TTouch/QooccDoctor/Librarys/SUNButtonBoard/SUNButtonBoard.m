...
- (id)init{
    ...
    if (self) {
        ...
        _boardWindow.windowLevel = UIWindowLevelNormal + 1;
        ...
        _boardView.boardSize = _boardSize;
...
- (void)setBoardImageString:(NSString *)boardImageString{
    _boardView.backgroundImage = boardImageString;
    _boardView.backgroundImageView.image = [UIImage imageNamed:boardImageString];
}

- (void)setBoardSelectedImage:(NSString *)boardSelectedImage{
    _boardView.backgroundSelectedImage = boardSelectedImage;
}

- (NSString*)boardImageString{
    return _boardView.backgroundImage;
}

- (NSString *)boardSelectedImage{
    return _boardView.backgroundImage;
}
...
- (void)buttonAction:(UIButton *)sender{
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:SUNButtonBoarButtonClickNotification object:[NSNumber numberWithInteger:sender.tag-100]]];
...
- (void)boardOpen{
    ...
    //处理_buttonNumber.count = 0 时创建按钮
    if (_buttonArray.count == 0) {
        [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:SUNButtonBoarButtonClickNotification object:nil]];
        [self boardClose];
    }else{
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    if (self.backgroundSelectedImage) {
        self.backgroundImageView.image = [UIImage imageNamed:self.backgroundSelectedImage];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_moving) {
        ...
        self.frame = CGRectMake(0, 0, self.boardSize, self.boardSize);
        
        
        if (self.selfBoard.autoPosition) {
            NSInteger direction = NSIntegerMax;
            ...
            CGRect newRect;
            //addBy--肖小丰 （不让按钮弹出屏幕外）
            CGFloat newRectOriginX = self.window.frame.origin.x;
            CGFloat newRectOriginY = self.window.frame.origin.y;
            if (newRectOriginX < 0){
                newRectOriginX = 0;
            }else if (newRectOriginX > [[UIScreen mainScreen] bounds].size.width - self.boardSize){
                newRectOriginX = [[UIScreen mainScreen] bounds].size.width - self.boardSize;
            }
            if (newRectOriginY < 0){
                newRectOriginY = 0;
            }else if(newRectOriginY > [[UIScreen mainScreen] bounds].size.height - self.boardSize){
                newRectOriginY = [[UIScreen mainScreen] bounds].size.height - self.boardSize;
            }
            
            switch (direction) {
                case 0:
                    newRect = CGRectMake(newRectOriginX,
                                         0,
                                         self.boardSize,
                                         self.boardSize);
                    break;
