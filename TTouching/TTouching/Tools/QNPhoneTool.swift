...
class QNPhoneTool: NSObject {
    ...
    */
    ...
    func tel(notification: NSNotification) {
        ...
        if let phone =  g_currentUserInfo?.phone, let phoneUrl = NSURL(string: "tel://" + phone) where phone != "" {
            let iphoneAlertView = UIAlertView(title: "确认要拨打" + g_currentUserInfo!.name + "的电话吗？", message: "电话：" + phone, delegate: nil, cancelButtonTitle: "取消")
